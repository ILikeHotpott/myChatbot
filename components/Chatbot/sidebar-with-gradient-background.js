...
import {Avatar, Button, ScrollShadow, Spacer, useDisclosure} from "@nextui-org/react";
 ...
 * ```
 */
export default function Component({children, header, title = "Yitong's Chatbot"}) {
    ...
    <div className="relative flex h-full w-72 flex-1 flex-col bg-gradient-to-b from-default-100 via-danger-100 to-secondary-100 p-6">
      ...
      <div className="flex flex-col gap-4">
        <div className="flex items-center gap-3 px-2">
          <Avatar size="sm" src="https://musictop-bucket.s3.ap-southeast-2.amazonaws.com/head.jpg" />
          <div className="flex flex-col">
            <p className="text-small text-foreground">Yitong Liu</p>
            <p className="text-tiny text-default-500">Full Stack Developer</p>
          </div>
        </div>
      </div>
    ...
    <div className="flex h-dvh w-full">
      ...
      <div className="flex w-full flex-col gap-y-4 p-4 sm:max-w-[calc(100%_-_288px)]">
        ...
        <main className="flex h-full">
          <div className="flex h-full w-full flex-col gap-4 rounded-medium  border-divider px-6">
            {children}
          </div>
        </main>
      </div>
